...
def _make_mock_encoders():
    ...
    le_primary.classes_ = np.array(
        ["ARSON", "BATTERY", "BURGLARY", "HOMICIDE", "ROBBERY", "THEFT"]
    )

    le_location = MagicMock()
    le_location.transform.return_value = np.array([10])
    le_location.classes_ = np.array(
        ["ALLEY", "APARTMENT", "RESIDENCE", "SIDEWALK", "STREET"]
    )
...
def test_predict_returns_float_in_range(sample_record: dict) -> None:
    ...
    with patch(
        "src.ml.predict.load_production_model",
