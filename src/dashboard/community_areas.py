"""Mapeo oficial community area (número) -> nombre — Chicago, 77 áreas fijas."""
...
NAME_TO_AREA: dict[str, int] = {v: k for k, v in COMMUNITY_AREAS.items()}


def code_to_name(code: object) -> str:
    """Convierte código numérico de community area a nombre legible."""
    try:
        return COMMUNITY_AREAS.get(int(float(str(code))), str(code))
